...
const genPreview = (args = {}) => {
  ...
  orderAmounts.map((amount, i) => {
    ...
    const randomDistortedPercentage = nBN(Math.random()).multipliedBy(intervalDistortion).toNumber() * m + 1
    const interval = intervalDistortion === 0
      ? sliceInterval
      : nBN(sliceInterval).multipliedBy(randomDistortedPercentage).toNumber()
