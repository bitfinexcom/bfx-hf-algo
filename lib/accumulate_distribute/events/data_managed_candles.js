  ...
  if (hasOffsetIndicator && chanTF === relativeOffset.candleTimeFrame) {
    if (!offsetIndicator.isSeeded()) {
      ...
      candles.forEach((candle) => {
        ...
        candleData.push(filterCandleData(candle, 'relative offset', relativeOffset, offsetIndicator))
      })
      ...
      candleData.push(filterCandleData(lastCandle, 'relative offset', relativeOffset, offsetIndicator, !updateLastCandleOffset))
    }
  ...
  if (hasCapIndicator && chanTF === relativeCap.candleTimeFrame) {
    if (!capIndicator.isSeeded()) {
      ...
      candles.forEach((candle) => {
        ...
        candleData.push(filterCandleData(candle, 'relative cap', relativeCap, capIndicator))
      })
      lastCandle = _last(candles)
    } else {
      ...
      candleData.push(filterCandleData(lastCandle, 'relative cap', relativeCap, capIndicator, !updateLastCandleCap))
    }
