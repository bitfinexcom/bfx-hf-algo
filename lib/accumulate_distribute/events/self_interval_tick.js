...
const hasRequeuedOrders = require('../../util/has_requeue_orders')
...
const onSelfIntervalTick = async (instance = {}) => {
  ...
  const { awaitFill, cancelDelay, submitDelay } = args
  ...
  if (hasOpenOrders(orders)) { // prev order still open
    ...
    } else if (hasRequeuedOrders(orders)) {
      await emit('exec:order:requeue:all', gid, orders, submitDelay)
      return
    } else {
      debug('awaiting fill...')
