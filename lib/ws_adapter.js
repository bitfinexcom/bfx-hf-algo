...
module.exports = class AOAdapter extends EventEmitter {
  ...
  findChannelId (connection, channel, filter) {
    const channelId = findChannelId(connection.c, (data) => {
  ...
  subscribe (connection, channel, filter) {
    const existChannelId = this.findChannelId(connection, channel, filter)

    if (!existChannelId) {
      return subscribe(connection.c, channel, filter)
    }
  }
  ...
  unsubscribe (connection, channel, filter) {
    ...
    unsubscribe(connection.c, existChannelId)
  }
