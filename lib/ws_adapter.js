...
const {
  subscribe, unsubscribe, findChannelId, Manager, cancelOrder: _cancelOrder,
  cancelOrdersByGid: _cancelOrdersByGid, submitOrder, send, ping
} = require('bfx-api-node-core')

const genCid = require('./util/gen_client_id')

const { HB_INTERVAL_MS, WD_PACKET_DELAY } = require('./constants')
...
module.exports = class AOAdapter extends EventEmitter {
  ...
  openWS () {
    ...
    this.m = new Manager({
      plugins: [
        ...
        ManagedOB(),
        ManagedCandles(),
        Watchdog({ packetWDDelay: this.packetWDDelay })
      ],
      transform: true,
      dms,
      apiSecret,
    ...
    this.m.on('ws2:message', this.propagateDataEvent.bind(this, 'message'))
  ...
  restartConnection (packetWDDelay) {
    ...
    // disconnect current ws connection
    this.disconnect()
  ...
  async submitOrder (connection, order) {
    ...
    return submitOrder(c, order)
  }

  async cancelOrder (connection, order) {
    const { c } = connection
    return new Promise((resolve, reject) => {
      _cancelOrder(c, order)
        .then(resolve)
        .catch(reject)
    })
  }

  async cancelOrdersByGid (connection, gid) {
    const { c } = connection
    return new Promise((resolve, reject) => {
      _cancelOrdersByGid(c, { gid })
        .then(resolve)
        .catch(reject)
    })
  }
