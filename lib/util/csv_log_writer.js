...
class CsvLogWriter {
  get _logDir () {
    return path.join(this.logAlgoDir, `${this.id}_logs`)
  }

  constructor ({ id, logAlgoDir, headersForLogFile }) {
    this.id = id
  ...
  async initFile (name) {
    const { logStreams, headersForLogFile } = this
    ...
    if (!logFileExists && Array.isArray(headersForLogFile) && headersForLogFile.length) {
      ...
      logStreams[name].write(headersForLogFile.join(',') + '\n')
    }
  }

  async write (filename, values) {
