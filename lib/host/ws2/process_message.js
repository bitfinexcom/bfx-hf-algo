  ...
  switch (type) {
    ...
    case 'order:close': {
      ...
      if (status === 'POSTONLY CANCELED') {
        /**
         * Triggered on postonly order cancellation
         *
         * @event AOHost~ordersOrderRequeued
         * @param {object} order - the order that was postonly cancelled
         */
        await triggerOrderEvent('orders', 'order_requeue', order)
      } else if (status.match(/CANCELED/)) {
        /**
