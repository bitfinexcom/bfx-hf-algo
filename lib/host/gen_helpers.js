  ...
  return {
    ...
    submitOrderWithDelay: async (state = {}, delay, o, attempt = 1) => {
      ...
      adapter.submitOrderWithDelay(connection, delay, o).then((order) => {
        ...
        debug('%s', notification.text ? notification.text : notification)

        if (/minimum size/.test(notification.text)) {
          await state.ev.emit('error:minimum_size', gid, o, notification)
        } else if (/balance/.test(notification.text)) {
          await state.ev.emit('error:insufficient_balance', gid, o, notification)
        } else if (/evaluate/.test(notification.text)) {
          if (attempt === MAX_SUBMIT_ATTEMPTS) {
            await state.ev.emit('error:evaluate_balance', gid, o, notification)
          } else {
