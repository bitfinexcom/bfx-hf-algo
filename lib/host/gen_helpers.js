...
const genHelpers = (state = {}, adapter) => {
  ...
  return {
     ...
     */
    close: () => {
      adapter.m.removeAllListeners(EVENT_SUBSCRIBED_CHANNEL)
    ...
    cancelOrdersByGid: async (state = {}, gid) => {
      ...
      adapter.cancelOrdersByGid(connection, gid).then(() => { // eslint-disable-line
        debug(
     ...
     */
    submitOrder: async (state = {}, order, attempt = 1) => {
      logHelperCall(
      ...
      if (attempt > MAX_SUBMIT_ATTEMPTS) { // note state assumed already patched
        // TODO cancel/delete from state an order defined in /submit_all_orders.js
        return state
      }

      const { connection } = state

      await adapter.submitOrder(connection, order).then((order) => { // eslint-disable-line
        debug(
        ...
        if (/minimum size/.test(text)) {
          ...
          if (attempt === MAX_SUBMIT_ATTEMPTS) {
            ...
            await adapter.submitOrder(connection, order)
          }
        } else {
