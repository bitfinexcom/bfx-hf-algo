...
const genHelpers = (state = {}, adapter) => {
  ...
  const pendingSubscriptionResponses = new Map()

  adapter.m.on(EVENT_SUBSCRIBED_CHANNEL, (event) => {
    const subscription = Array.from(pendingSubscriptionResponses.values())
  ...
  const helpers = {
    ...
    close: () => {
      ...
      for (const [eid, pendingSubscription] of pendingSubscriptionResponses.entries()) {
    ...
    subscribeDataChannels: (state = {}, options = {}) => {
      ...
      return Promise.all(
        channels.map(({ channel, filter: payload }) => {
          ...
          pendingSubscriptionResponses.set(eid, {
            eid,
            channel,
            payload,
            resolve,
