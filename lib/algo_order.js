...
class AlgoOrder extends EventEmitter {
  ...
  constructor (ws, rest, args = {}, uiSpec) {
    ...
    this.active = args.active || false // manual start() required even if true
    this.atomicOrders = {}
  ...
  start (opts = {}) {
    ...
    if (this.active) {
      return Promise.reject(new Error('algo order already running'))
    }
  ...
  stop (opts = {}) {
    ...
    if (!this.active) {
      return Promise.reject(new Error('algo order not running'))
    }
  ...
  static getLimits (currency) {
    if (!LIMITS[currency]) {
      if (!LIMIT_WARNING_SHOWN[currency]) {
        AODebug(`warning: no limit configured for ${currency}`)
