...
const unregisterAOUIs = require('./host/ui/unregister_ao_uis')
...
class AOHost extends AsyncEventEmitter {
   ...
   * 
   * @param  {string} id - i.e. bfx-iceberg
   * @return {undefined} - void method 
   */
  disableAO(id) {
     unregisterAOUIs(this, id)
  }
  /**
  ...
  changeAOStatus (label) {
    registerAOUIs(this, label).catch((err) => {
      debug('failed to register AO UIs: %s', err.message)
    })
  }
  /**
