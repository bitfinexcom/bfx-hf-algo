...
const onResubmitAllOrders = require('./host/events/resubmit_all_orders')
...
class AOHost extends AsyncEventEmitter {
  ...
  async bootstrapAO (ao, instance = {}, gidCB) {
    ...
    state.ev.on('exec:order:requeue:all', onResubmitAllOrders.bind(null, this))
  ...
  async triggerOrderEvent (section, eventName, order) {
    ...
    await PI.forEach(instances, async (instance) => {
      ...
      if (eventName === 'order_requeue') order.status = 'REQUEUE'
