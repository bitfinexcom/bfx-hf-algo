...
class AOHost extends AsyncEventEmitter {
  ...
  constructor (args = {}) {
    ...
    this.adapter.on('data:message', this.onDataMessage.bind(this))
    ...
    this.packetWDDelay = wsSettings.packetWDDelay || WD_PACKET_DELAY
  ...
  /**
   * Disconnect & reconnect the exchange adapter with updated packet delay
   */
  restartConnection (packetWDDelay) {
    if (packetWDDelay) {
      this.packetWDDelay = packetWDDelay
      this.adapter.restartConnection(packetWDDelay)
    }
  }
