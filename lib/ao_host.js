...
class AOHost extends AsyncEventEmitter {
   ...
   * @param {object} [args.db] - `bfx-hf-models` DB
   * @param {string} [args.wsURL='wss://api.bitfinex.com/ws/2'] - WS endpoint
   * @param {string} [args.restURL='https://api.bitfinex.com'] - REST endpoint
   * @param {object} [args.agent] - proxy agent
   * @param {object[]} [args.aos] - algo orders to manage
   * @param {number} [args.dms=4] - dead man switch, active 4
   */
  constructor (args = {}) {
    super()
    ...
    this.adapter.once('order:snapshot', (snapshot) => {
      ...
      this.loadAllAOs().then(() => { // eslint-disable-line
        this.emit('ready')
   ...
   *
   * @returns {AOInstance[]} aoInstances
   */
  getAOInstances () {
   ...
   *
   * @param {bfx-api-node-models.Notification} notification - model
   ...
   * @param {AOInstance} instance - new algo order to be started
   * @param {Function} [gidCB] - callback to acquire GID prior to ao:start
   ...
   * @param {...(object|Array|string|number)} args - event arguments
  ...
  /**
   * @returns {boolean} aosRunning - true if any algo order is currently
   *   running
   */
  aosRunning () {
  ...
  /**
   * Handles init for an algo order instance; sets the 'active' flag,
   * subscribes to required channels, and triggers the life.start event.
   *
