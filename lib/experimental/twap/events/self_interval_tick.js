...
module.exports = async (instance = {}) => {
  ...
  const { emit, debug } = h
  const {
    priceTarget, tradeBeyondEnd, cancelDelay, submitDelay
  } = args
  ...
  if (!targetMet) {
    debug('tick | target not met | last price %f (target %f)', orderPrice, priceTarget)
    return
  }

  debug('tick | target met | last price %f (target %f)', orderPrice, priceTarget)
