'use strict'

const { preparePrice } = require('bfx-api-node-util')

module.exports = (state = {}) => {
  const { bbandsIndicator, args = {} } = state
  const { bbandsDistance, orderCount, pongAmount } = args
  const pingPongTable = {}

  const bb = bbandsIndicator.v()
  const pingMaxPrice = bb.top
  const pingMinPrice = bb.bottom
  const step = (pingMaxPrice - pingMinPrice) / (orderCount - 1)

  for (let i = 0; i < orderCount; i += 1) {
    const price = pingMinPrice + (i * step)
    pingPongTable[preparePrice(price)] = preparePrice(pongAmount > 0
      ? price + bbandsDistance
      : price - bbandsDistance)
  }

  return pingPongTable
}
