...
module.exports = async (instance = {}) => {
  ...
  const { amount, submitDelay, symbol, hidden, _margin } = args
  ...
  if (activePongs.length === 0) {
    return
  }

  // Handle saved pongs
  const pongOrders = activePongs.map(price => (
    new Order({
      symbol,
      price,
      cid: nonce(),
  ...
  debug('submitting pong orders: [%j]', activePongs)
