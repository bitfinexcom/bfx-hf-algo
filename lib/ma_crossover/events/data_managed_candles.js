...
const filterCandleData = (candle, indicatorType, indicatorTypeArgs, indicator, candleIgnored = false) => {
  return [indicatorType, new Date(candle.mts), candle[indicatorTypeArgs.candlePrice], indicatorTypeArgs.candlePrice, indicator.getName(), indicator.v(), +candleIgnored]
}
...
const onDataManagedCandles = async (instance = {}, candles, meta) => {
  ...
  const { debug, updateState, emitSelf, emit, tracer } = h
  ...
  if (chanTF === long.candleTimeFrame) {
    if (!longIndicator.isSeeded()) {
      ...
      candleData.push(filterCandleData(lastCandle, 'long', long, longIndicator, !updateLastCandleLong))
    }
  ...
  if (chanTF === short.candleTimeFrame) {
    if (!shortIndicator.isSeeded()) {
      ...
      candleData.push(filterCandleData(lastCandle, 'short', short, shortIndicator, !updateLastCandleShort))
    }
  ...
  const signal = tracer.createSignal('data_candles', null, {
  ...
  if (indicatorsUpdated) {
    ...
    if (_isFinite(shortV) && _isFinite(longV) && (
      ...
      await emitSelf('submit_order', signal)
      await emit('exec:stop', null, { keepOrdersOpen: true, origin: signal })
    }
  }
}
